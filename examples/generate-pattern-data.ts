...
const OUTPUT_DIR = path.join(__dirname, '../src/streamer/data/pattern_experiments');
...
interface PatternConfig {
  name: string;
  description: string;
  generator: (index: number, totalObservations: number) => number;
}
...
const PATTERNS: PatternConfig[] = [
  ...
  },
  {
    ...
    generator: (i: number, total: number) => {
      ...
      const baseValue = i < midpoint ? -23.0 : -15.0;
  ...
  },
  {
    ...
    generator: (i: number, total: number) => {
      const baseValue = -23.0;
      const spikeStart = Math.floor(total / 2) - 2;
      const spikeEnd = Math.floor(total / 2) + 2;
  ...
  },
  {
    ...
    generator: (i: number, total: number) => {
      ...
      const oscillation = amplitude * Math.sin(2 * Math.PI * frequency * i);
  ...
  },
  {
    ...
    generator: (i: number, total: number) => {
      ...
      const endValue = -15.0;
      const drift = startValue + (endValue - startValue) * (i / total);
      const noise = (Math.random() - 0.5) * 0.3;
      return drift + noise;
    }
  },
...
function generateObservationQuads(
  ...
  const sensorId = deviceType === 'wearable' ? 'E4.A03846.Accelerometer' : 'Samsung.Galaxy.S21.Accelerometer';
...
async function generateAllPatterns(): Promise<void> {
  ...
  for (const pattern of PATTERNS) {
    ...
    const metadata = {
      ...
      end_timestamp: new Date(BASE_TIMESTAMP + DURATION_SECONDS * 1000).toISOString(),
  ...
  console.log(`\n=== Generation Complete ===`);
  console.log(`Generated ${PATTERNS.length} patterns with wearable and smartphone data.`);
  console.log(`Total files created: ${PATTERNS.length * 3} (data.nt × 2 + metadata.json per pattern)`);
}

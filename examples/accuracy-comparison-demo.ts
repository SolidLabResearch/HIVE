...
/**
 * Runs the accuracy comparison demo between approximation and fetching client side approaches.
 */
async function runAccuracyComparison() {
  ...
  mqttClient.on("connect", () => {
    ...
    mqttClient.subscribe(APPROX_RESULT_TOPIC, (err) => {
      if (err)
        console.error(`Failed to subscribe to ${APPROX_RESULT_TOPIC}`, err);
    ...
    mqttClient.subscribe(GROUND_TRUTH_RESULT_TOPIC, (err) => {
      if (err)
        console.error(
          `Failed to subscribe to ${GROUND_TRUTH_RESULT_TOPIC}`,
          err,
        );
  ...
  const approxOrchestrator = spawn("npx", [
    "ts-node",
    "src/approaches/ApproximationApproachOrchestrator.ts",
  ]);
  const fetchingOrchestrator = spawn("npx", [
    "ts-node",
    "src/approaches/FetchingClientSideApproachOrchestrator.ts",
  ]);

  // Log output from child processes
  approxOrchestrator.stdout.on("data", (data) =>
    console.log(`[Approx Orch]: ${data}`),
  );
  approxOrchestrator.stderr.on("data", (data) =>
    console.error(`[Approx Orch ERROR]: ${data}`),
  );
  fetchingOrchestrator.stdout.on("data", (data) =>
    console.log(`[Fetching Orch]: ${data}`),
  );
  fetchingOrchestrator.stderr.on("data", (data) =>
    console.error(`[Fetching Orch ERROR]: ${data}`),
  );
...
<http://example.org/sensor/1> <https://saref.etsi.org/core/relatesToProperty> <https://dahcc.idlab.ugent.be/Homelab/SensorsAndActuators/wearableX> .`;
  ...
  if (approxResults.length > 0 && groundTruthResults.length > 0) {
    ...
    const mape =
      (errors.reduce((a, b, i) => a + b / truth[i], 0) / truth.length) * 100;

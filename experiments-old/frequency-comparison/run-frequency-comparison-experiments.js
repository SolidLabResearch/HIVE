...
async function runFrequencyExperiments() {
  ...
  console.log(
    `Testing ${FREQUENCIES.length} frequencies × ${APPROACHES.length} approaches = ${FREQUENCIES.length * APPROACHES.length} total experiments`,
  );
      ...
      try {
        ...
        const scriptName =
          approach === "approximation"
            ? "experiments/frequency-comparison/experiment-frequency-comparison-approximation.js"
            : "experiments/frequency-comparison/experiment-frequency-comparison-fetching.js";
        ...
        console.error(
          `Failed experiment: ${approach} complex oscillation ${frequency} Hz`,
        );
  ...
  console.log(
    "- 1.0 Hz: Medium frequency, adequately sampled (4 samples/cycle)",
  );
  console.log(
    "- 1.5 Hz: High frequency, medium aliasing risk (2.7 samples/cycle)",
  );
  console.log("- 2.0 Hz: Near Nyquist, high aliasing risk (2.0 samples/cycle)");

  console.log("\nNext steps:");
  console.log(
    "1. Check logs/frequency-comparison-* directories for detailed results",
  );

 ...
 * Tests only exponential growth/decay patterns with different rates using chunked approach
 */
...
const LOGS_DIR = "logs/rate-comparison-chunked";
...
async function runAllRateTests() {
  console.log("Starting Exponential Rate Comparison - Chunked Approach");
  ...
  const results = [];
  const totalTests =
    RATE_TEST_CONFIG.rates.length *
    RATE_TEST_CONFIG.patterns.length *
    RATE_TEST_CONFIG.iterations;
  ...
  for (const rate of RATE_TEST_CONFIG.rates) {
    for (const pattern of RATE_TEST_CONFIG.patterns) {
      for (
        let iteration = 1;
        iteration <= RATE_TEST_CONFIG.iterations;
        iteration++
      ) {
        try {
          ...
          console.error(
            `💥 Test failed: ${pattern} rate ${rate} iteration ${iteration}`,
            error.message,
          );
          results.push({
            rate,
...
async function runSpecificRate(rate) {
  if (!RATE_TEST_CONFIG.rates.includes(rate)) {
    throw new Error(
      `Invalid rate: ${rate}. Valid rates: ${RATE_TEST_CONFIG.rates.join(", ")}`,
    );
  }

  console.log(`Testing Chunked Approach for rate: ${rate}`);
...
async function runSpecificTest(pattern, rate) {
  if (!RATE_TEST_CONFIG.patterns.includes(pattern)) {
    throw new Error(
      `Invalid pattern: ${pattern}. Valid patterns: ${RATE_TEST_CONFIG.patterns.join(", ")}`,
    );
  }

  console.log(`Testing Chunked Approach: ${pattern} with rate ${rate}`);
...
function generateSummaryReport(results) {
  ...
  const reportPath = path.join(
    LOGS_DIR,
    "fetching_rate_comparison_summary.json",
  );
  fs.writeFileSync(
    reportPath,
    JSON.stringify(
...
async function main() {
  ...
  try {
    if (args.length === 0) {
      ...
      console.log("Usage:");
      console.log(
        "  node experiment-rate-comparison-fetching.js                           # Run all rate tests",
      );
      console.log(
        "  node experiment-rate-comparison-fetching.js rate 0.1                 # Run tests for specific rate",
      );
      console.log(
        "  node experiment-rate-comparison-fetching.js test exponential_growth 1 # Run specific test",
      );
      process.exit(1);
    }
  } catch (error) {

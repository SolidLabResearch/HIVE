...
function compareFrequency(frequency) {
  ...
  // Try to find the correct directory with different naming conventions
  const possibleDirNames = [
    `${DATASET_TYPE}_freq_${frequency}`,
    `${DATASET_TYPE}_freq_${frequency}_freq_${frequency}`,
  ];

  let fetchingDir = null;
  let approximationDir = null;

  // Find fetching directory
  ...
  // Find approximation directory
  for (const dirName of possibleDirNames) {
    const testDir = path.join(
      LOG_BASE_DIR,
      APPROXIMATION_LOG_DIR,
      dirName,
      "iteration1",
    );
    if (fs.existsSync(testDir)) {
      approximationDir = testDir;
      break;
    }
  }

  if (!fetchingDir) {
    console.log(`Fetching directory not found for frequency ${frequency} Hz`);
    return null;
  }

  if (!approximationDir) {
    console.log(
      `Approximation directory not found for frequency ${frequency} Hz`,
    );
    return null;
  }

  // NEW: Use the captured results CSV files instead of approach logs
  const fetchingResultsPath = path.join(fetchingDir, "fetching_results.csv");
  const approximationResultsPath = path.join(
    approximationDir,
    "approximation_results.csv",
  );

  // Check if results files exist
  if (!fs.existsSync(fetchingResultsPath)) {
    console.log(`Fetching results not found: ${fetchingResultsPath}`);
    console.log(`   (Make sure to run experiments with capture enabled)`);
    return null;
  }

  if (!fs.existsSync(approximationResultsPath)) {
    console.log(`Approximation results not found: ${approximationResultsPath}`);
    console.log(`   (Make sure to run experiments with capture enabled)`);
    return null;
  }

  console.log(`Parsing ground truth (fetching): ${fetchingResultsPath}`);
  const fetchingData = parseCSVFile(fetchingResultsPath);

  console.log(`Parsing approximation data: ${approximationResultsPath}`);
  const approximationData = parseCSVFile(approximationResultsPath);
  ...
  // Extract result values from the captured results CSV
  // The CSV has columns: timestamp,window_number,result_value,latency_from_start_ms
  let fetchingValues = extractNumericalValues(fetchingData, "result_value");
  let approximationValues = extractNumericalValues(
    approximationData,
    "result_value",
  );

  if (fetchingValues.length === 0) {
    console.log("No valid fetching values found in result_value column");
    console.log("Available columns:", Object.keys(fetchingData[0] || {}));
    return null;
  }

  if (approximationValues.length === 0) {
    console.log("No valid approximation values found in result_value column");
    console.log("Available columns:", Object.keys(approximationData[0] || {}));
    return null;
  }

  console.log(`✓ Extracted ${fetchingValues.length} fetching values`);
  console.log(`✓ Extracted ${approximationValues.length} approximation values`);
  ...
  console.log(
    `   • MAPE (Mean Absolute Percentage Error): ${results.accuracy.mape}%`,
  );
...
function main() {
  console.log(
    "Accuracy Comparison: Approximation vs Fetching Client Side Approach",
  );
  console.log(
    "=========================================================================",
  );
  ...
  console.log(
    "Analyzing Complex Oscillation data across different frequencies\n",
  );
  ...
  console.log("================");
  console.log(
    "Freq(Hz) | Data Points | MAPE(%)  | MAE      | RMSE     | Correlation",
  );
  console.log(
    "---------|-------------|----------|----------|----------|------------",
  );
  ...
  if (validResults.length > 0) {
    ...
    const correlationValues = validResults
      .map((r) => parseFloat(r.accuracy.correlation))
      .filter((v) => !isNaN(v));
    ...
    if (avgCorrelation !== null) {
      console.log(
        `Average correlation across all frequencies: ${avgCorrelation.toFixed(4)}`,
      );
    }
    ...
    const bestFreq = validResults.find(
      (r) => parseFloat(r.accuracy.mape) === bestMAPE,
    ).frequency;
    const worstFreq = validResults.find(
      (r) => parseFloat(r.accuracy.mape) === worstMAPE,
    ).frequency;

    console.log(
      `🏆 Best accuracy (lowest MAPE): ${bestFreq} Hz (${bestMAPE.toFixed(4)}%)`,
    );
    console.log(
      ` Worst accuracy (highest MAPE): ${worstFreq} Hz (${worstMAPE.toFixed(4)}%)`,
    );

    // Aliasing analysis
    const highFreqResults = validResults.filter(
      (r) => parseFloat(r.frequency) >= 1.5,
    );
    if (highFreqResults.length > 0) {
      const highFreqMAPE = highFreqResults.map((r) =>
        parseFloat(r.accuracy.mape),
      );
      const avgHighFreqMAPE =
        highFreqMAPE.reduce((a, b) => a + b, 0) / highFreqMAPE.length;

      console.log(
        `High frequency (≥1.5 Hz) average MAPE: ${avgHighFreqMAPE.toFixed(4)}%`,
      );
      if (avgHighFreqMAPE > avgMAPE) {
        console.log(
          ` Approximation accuracy degrades at high frequencies (aliasing effect)`,
        );
      }
    }
  }

...
export class CSPARQLWindow {
  ...
  constructor(
    ...
    const log_level: LogLevel =
      LogLevel[LOG_CONFIG.log_level as keyof typeof LogLevel];
  ...
  getContent(timestamp: number): QuadContainer | undefined {
    ...
    this.active_windows.forEach(
      (value: QuadContainer, window: WindowInstance) => {
  ...
  add(event: Quad, timestamp: number): void {
    ...
    console.debug(
      `Adding [" + ${event} + "] at time : ${timestamp} and watermark ${this.current_watermark}`,
    );
    const t_e = timestamp;
    ...
    if (this.time > timestamp) {
      if (!this.enableOOO) {
        ...
        console.debug(
          `Out-of-order processing is disabled. Ignoring event [" + ${event} + "] at time : ${timestamp}`,
        );
        return;
      }
      ...
      console.error(
        `The event is late and has arrived out of order at time ${timestamp}`,
      );
        ...
        for (const w of Array.from(this.active_windows.keys())) {
          if (w.open <= t_e && t_e < w.close) {
            ...
            if (temp_window) {
              ...
              this.logger.info(
                `adding_out_of_order_event ${event.subject.value} to the window ${this.name} with bounds ${w.getDefinition()} at time ${timestamp}`,
                `CSPARQLWindow`,
              );
      ...
      for (const w of Array.from(this.active_windows.keys())) {
        console.debug(
          `Processing Window ${w.getDefinition()} for the event ${event} at time ${timestamp}`,
        );
        if (w.open <= t_e && t_e < w.close) {
          console.debug(
            `Adding the event ${event} to the window ${w.getDefinition()} at time ${timestamp}`,
          );
          const window_to_add = this.active_windows.get(w);
          if (window_to_add) {
            this.logger.info(
              `adding_in_order_event ${event.subject.value} to the window ${this.name} with bounds ${w.getDefinition()} at time ${timestamp}`,
              `CSPARQLWindow`,
            );
          ...
          console.debug(
            `Scheduled to evict the window ${w.getDefinition()} at time ${timestamp}`,
          );
          to_evict.add(w);
        }
      }
  ...
  addInOrder(event: Quad, timestamp: number): void {
    ...
    console.debug(
      `Adding in-order event [" + ${event} + "] at time : ${timestamp} and watermark ${this.current_watermark}`,
    );

    if (timestamp < this.time) {
      this.logger.info(
        `in_order_event_ignored_late_timestamp`,
        `CSPARQLWindow`,
      );
      console.debug(
        `Ignoring in-order event with timestamp ${timestamp} as it is earlier than current time ${this.time}`,
      );
      return;
    }
  ...
  addOutOfOrder(event: Quad, timestamp: number): void {
    this.logger.info(
      `adding_out_of_order_event_to_the_window`,
      `CSPARQLWindow`,
    );
    console.debug(
      `Adding out-of-order event [" + ${event} + "] at time : ${timestamp} and watermark ${this.current_watermark}`,
    );

    if (timestamp >= this.time) {
      this.logger.info(
        `out_of_order_event_ignored_early_timestamp`,
        `CSPARQLWindow`,
      );
      console.debug(
        `Ignoring out-of-order event with timestamp ${timestamp} as it is not earlier than current time ${this.time}`,
      );
      return;
    }
  ...
  trigger_window_content(watermark: number, timestamp: number): void {
    ...
    if (max_window) {
      if (this.tick == Tick.TimeDriven && watermark >= max_time) {
          ...
          if (max_window && max_window.has_triggered === false) {
            if (watermark >= max_time + this.max_delay) {
              ...
              if (windowToDelete) {
                this.emitter.emit(
                  "RStream",
                  this.active_windows.get(windowToDelete),
                );
                this.logger.info(
                  `Window with bounds [${windowToDelete.open},${windowToDelete.close}) ${windowToDelete.getDefinition()} is triggered for the window name ${this.name}`,
                  `CSPARQLWindow`,
                );
        ...
        this.logger.info(
          `Window ${max_window} is out of the watermark and will not trigger.`,
          `CSPARQLWindow`,
        );
  ...
  update_watermark(new_time: number): void {
    if (new_time > this.current_watermark) {
      ...
      this.logger.info(
        `Watermark is increasing ${this.current_watermark} and time ${this.time}`,
        `CSPARQLWindow`,
      );
    } else {
   ...
   */
  subscribe(
    output: "RStream" | "IStream" | "DStream",
    call_back: (data: QuadContainer) => void,
  ) {

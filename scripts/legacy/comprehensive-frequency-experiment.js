...
async function runSingleExperiment(approach, frequency, device, iteration) {
    ...
    try {
        ...
        console.log(`  [OK] Completed ${runId} in ${(duration/1000).toFixed(1)}s`);
        ...
        console.error(`  [FAIL] Error in ${runId}:`, error.message);
        killLingeringProcesses();
...
async function main() {
    ...
    try {
        ...
        for (const approach of CONFIG.APPROACHES) {
            ...
            console.log(`\\n[OK] Completed all experiments for ${approach.name}`);

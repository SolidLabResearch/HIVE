...
import { FrequencyExperimentSetup } from "../setup/setup-frequency-experiment";
...
class QuickExperimentTest {
  ...
  public async runQuickTest(): Promise<void> {
    ...
    try {
      ...
      console.log("1. Running setup validation...");
      const setup = new FrequencyExperimentSetup();
      await setup.run();

      // Step 2: Test single combination
      console.log(
        "\n2. Testing single frequency-device-approach combination...",
      );
      ...
      console.log("\n3. Validating data loading across all frequencies...");
      await this.validateDataLoading();

      console.log("\nQuick test completed successfully!");
      console.log("\nNext Steps:");
      console.log(
        "• Run full experiment: ts-node tools/experiments/run-frequency-experiment.ts",
      );
      console.log(
        "• Analyze results: ts-node tools/experiments/analyze-frequency-results.ts",
      );
    } catch (error) {
      ...
      console.log(
        "• Verify frequency data exists in src/streamer/data/frequency_variants/2mins/",
      );
      process.exit(1);
    }
  }
  ...
  private async testSingleCombination(config: QuickTestConfig): Promise<void> {
    console.log(
      `Testing ${config.approach} with ${config.frequency} ${config.deviceType} data...`,
    );
    ...
    console.log(
      `  Data format validated (sample: ${sampleData.length} characters)`,
    );
  ...
  private async validateDataLoading(): Promise<void> {
    ...
    for (const frequency of frequencies) {
      for (const deviceType of deviceTypes) {
        ...
        const observationCount = this.countObservations(dataPath);
        let expectedCount =
          expectedCounts[frequency as keyof typeof expectedCounts];

        if (typeof expectedCount === "object") {
          expectedCount =
            expectedCount[deviceType as keyof typeof expectedCount];
        }

        if (observationCount !== expectedCount) {
          issues.push(
            `${deviceType}/${frequency}: Expected ${expectedCount}, got ${observationCount}`,
          );
        } else {
          console.log(
            `  ${deviceType}/${frequency}: ${observationCount} observations`,
          );
        }
      }
    }

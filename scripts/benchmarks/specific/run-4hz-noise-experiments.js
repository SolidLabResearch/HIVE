        ...
        for (const noise of NOISE_LEVELS) {
            for (let i = 1; i <= ITERATIONS; i++) {
                try {
                    ...
                    console.log(`[OK] Completed ${approach} noise ${noise} iteration ${i}`);
                } catch (error) {
                    console.error(`[FAIL] Failed ${approach} noise ${noise} iteration ${i}: ${error.message}`);
                }
            }
        }

        console.log(`[OK] Completed all ${approach} experiments`);
    }

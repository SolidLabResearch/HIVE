...
class FrequencyExperimentAnalyzer {
    ...
    public async analyzeResults(): Promise<void> {
        ...
        try {
            ...
            console.error('[FAIL] Analysis failed:', error);
            throw error;
        }
    }
    ...
    private generateRecommendations(report: AnalysisReport): string[] {
        ...
        if (report.experiment.successRate < 95) {
            recommendations.push(`[WARNING]  Success rate is ${report.experiment.successRate.toFixed(1)}%. Investigate failed experiments.`);
        }

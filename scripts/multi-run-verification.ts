...
/**
 * Multi-run verification test for all 3 streaming query approaches.
 * Runs the verification multiple times to ensure stability and consistency.
 */
...
const NUM_RUNS = 1; // Number of test iterations
...
class MultiRunOrchestrator {
  ...
  private saveResultsToFile(): void {
    const filename = `multi-run-results-${new Date().toISOString().replace(/:/g, "-").split(".")[0]}.json`;
    const data = {

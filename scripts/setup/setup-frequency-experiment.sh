...
if [[ ! -f "package.json" ]]; then
    echo "[FAIL] Error: Please run this script from the root of the streaming-query-hive project"
    exit 1
fi
...
if ! command_exists npm; then
    echo "[FAIL] Error: npm is not installed"
    exit 1
fi
...
if [[ ! -d "dist" ]]; then
    echo "[FAIL] Error: TypeScript compilation failed - dist directory not found"
    exit 1
fi
...
for file in "${REQUIRED_FILES[@]}"; do
    if [[ ! -f "$file" ]]; then
        echo "[WARNING]  Warning: $file not found - some approaches may not work"
    else
        echo "[OK] Found: $file"
    fi
done
...
if [[ ! -d "$DATA_DIR" ]]; then
    echo "[FAIL] Error: Frequency variant data not found at $DATA_DIR"
...
echo "[OK] Found frequency variants:"
...
if [[ $SMARTPHONE_COUNT -lt 6 ]] || [[ $WEARABLE_COUNT -lt 6 ]]; then
    echo "[WARNING]  Warning: Expected 6 frequency variants for each device, but found fewer"
...
echo "[OK] Analysis directory ready"

# Check if MQTT broker is available (optional)
echo " Checking MQTT broker availability..."
if command_exists mosquitto; then
    echo "[OK] Mosquitto MQTT broker is available"
    
    # Check if broker is running
    if pgrep -x "mosquitto" > /dev/null; then
        echo "[OK] MQTT broker is already running"
    else
        echo "[WARNING]  MQTT broker is not running. You may need to start it:"
        echo "   $ mosquitto -d"
    fi
else
    echo "[WARNING]  Warning: Mosquitto MQTT broker not found"

...
/**
 * Automated Real Data 3-Way Comparison Experiment (No prompts)
...
const ITERATIONS = 1; // Start with 1 iteration for testing
...
class RealDataComparisonRunner {
  ...
  async runAllTests() {
    console.log("🚀 Starting Real Data 3-Way Comparison (Automated)");
    ...
    for (const approach of APPROACHES) {
      for (let iteration = 1; iteration <= ITERATIONS; iteration++) {
        try {
          ...
          console.error(
            `💥 Test failed: ${approach.label} iteration ${iteration}`,
            error,
          );
          this.results.push({
            approach: approach.name,
  ...
  parseReplayerLog(logPath) {
    ...
    try {
      ...
      const summaryRecord = records.find(
        (r) => r.intended && r.successful && r.failed,
      );
      ...
      if (summaryRecord) {
        return {
          ...
          successRate:
            (parseInt(summaryRecord.successful) /
              parseInt(summaryRecord.intended)) *
            100,
        };
      }
  ...
  analyzeResults() {
    ...
    for (const approach of APPROACHES) {
      const approachResults = this.results.filter(
        (r) => r.approach === approach.name && r.success,
      );
      ...
      analysis.byApproach[approach.name] = {
        ...
        resultValues: allResultValues.length > 0 ? allResultValues[0] : [],
      };
    }
  ...
  generateReport(analysis) {
    ...
    const csvRows = [];
    csvRows.push(
      "Approach,Iterations,Avg_Latency_ms,Min_Latency_ms,Max_Latency_ms,Window_Count,Accuracy_%,MAE,MAPE_%",
    );
    ...
    for (const approach of APPROACHES) {
      ...
      if (!data) {
        console.log(
          `| ${approach.label.padEnd(20)} | No Data    |             |             |             |         |          |          |          |`,
        );
        continue;
      }
      ...
      console.log(
        `| ${label} | ${iterStr} | ${avgLatStr} | ${minLatStr} | ${maxLatStr} | ${winStr} | ${accStr} | ${maeStr} | ${mapeStr} |`,
      );
  ...
  generateSummary(analysis) {
    ...
    for (const approach of APPROACHES) {
      ...
      if (fetchingData && data.avgLatency && fetchingData.avgLatency) {
        ...
        latencyComparison =
          diff > 0 ? `(+${pct}% vs baseline)` : `(${pct}% vs baseline)`;
      }

      console.log(
        `${approach.label.padEnd(25)}: Avg Latency = ${avgLat} ms ${latencyComparison}`,
      );
    }
    ...
    const latencies = APPROACHES.map((a) => ({
      name: a.label,
      latency: analysis.byApproach[a.name]?.avgLatency,
    ...
    if (latencies.length > 0) {
      ...
      console.log(
        `🏆 Fastest approach: ${fastest.name} (${fastest.latency.toFixed(2)} ms avg)`,
      );
    }
  ...
  async run() {
    ...
    console.log("📋 Running automated experiment...");
    console.log("  ✓ MQTT broker should be running");
    console.log("  ✓ Data files should exist in src/streamer/data/");
    console.log("  ✓ Project should be built\n");

...
const APPROACHES = {
  ...
  "chunked-exact-final": {
    ...
    validationApproach: "chunked-exact-final",
  },
};
...
function buildResultRoot() {
  return path.join(
    ...
    "results",
    "paper-benchmarks",
    `h1-chunked-exact-final-1x-local-${sanitizeTimestamp(new Date())}`,
  );
}
...
function buildRunEnv({ approach, kValue, iteration, runRoot, timeoutMs, replayAnchor }) {
  ...
  return replayEnv.withBenchmarkReplayEnv({
    ...
    SESSION_ID: `h1_${approach}_K${kValue}_iteration${iteration}`,
    RESULT_TOPIC: `h1-chunked-exact-final/results/${approach}/K${kValue}/iteration${iteration}`,
    BENCHMARK_SCENARIO: "h1-chunked-exact-final",
    ...
    K_SCALING_REUSE_MODE: approach === "chunked-exact-final" ? "hierarchical-exact-final" : "chunk-state",
...
function validateStandardRun({ runRoot, approach, kValue }) {
  const validationApproach = APPROACHES[approach].validationApproach;
  const failures = [];
  const consumerFiles = countConsumerLatencyFiles(runRoot, validationApproach, kValue);
  const allConsumerWindows = extractAllConsumerWindows(runRoot, validationApproach, kValue);
  const representative = extractRepresentativeWindow(runRoot, validationApproach, kValue, 1);
  ...
  return {
    ...
    processTree,
    topology: {
      queryExecutionCount: approach === "fetching" ? kValue : kValue,
      deliveryEventCount: kValue,
      reconstructionWorkerCount: approach === "chunked-per-consumer" ? kValue : 0,
...
function validateRun({ runRoot, approach, kValue, expectedValue }) {
  const processTree = readProcessTreeMetrics(runRoot);
  if (approach === "chunked-exact-final") {
    const validation = validateHierarchicalReuseRun(runRoot, kValue, {
      expectedValue,
      valueTolerance: 1e-9,
    });
    return {
...
function shouldRequireStartupBarrier(approach) {
  return approach === "fetching" || approach === "chunked-exact-final";
}
  ...
  return new Promise((resolve) => {
    ...
    try {
      orchestrator = spawn("node", [orchestratorScript], {
        cwd: REPO_ROOT,
        env: { ...env, HIVE_PROCESS_ROLE: `${approach}_h1_orchestrator` },
...
async function main() {
  ...
  console.log(`H1 hierarchical reuse local smoke complete. Result root: ${resultRoot}`);
}
